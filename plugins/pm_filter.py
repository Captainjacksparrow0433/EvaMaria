...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons= [[
            InlineKeyboardButton('🎥 Ott Movie', url='https://t.me/filmbox112'),
...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if imdb and imdb.get('poster'):
            ...
            await message.reply_photo(photo=imdb.get('poster'), caption=f"""
Query: {search}
...
📤 Uploaded by: {query.message.chat.title}""",
 
      reply_markup=InlineKeyboardMarkup(btn))

        elif imdb:

            await message.reply_text(f"<b>Query: {search}</b> \n🏷 Title: <a href={imdb['url']}>{imdb.get('title')}</a>\n🎭 Genres: {imdb.get('genres')}\n📆 Year: <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🌟 Rating: <a href={imdb['url']}/ratings>{imdb.get('rating')}</a> / 10\n👤 Requested By: {query.from_user.mention}\n🖋 𝖲𝗍𝗈𝗋𝗒𝗅𝗂𝗇𝖾: <code>{imdb.get('plot')}</code>\n📤 Uploaded by: {query.message.chat.title}", reply_markup=InlineKeyboardMarkup(btn))
